...
class GraphMorphNonIntersect extends Panel implements MouseListener, MouseMotionListener, AdjustmentListener, ItemListener, ActionListener
{
    ...
    void drawNode(final Graphics graphics, final int n, final int n2, Color color, final int n3, final int n4) {
        ...
        color = new Color(255 - (255 - color.getRed()) * n4 / 100, 255 - (255 - color.getGreen()) * n4 / 100, 255 - (255 - color.getBlue()) * n4 / 100);
    ...
    void redrawGraph(final Graphics graphics) {
        ...
        for (int i = 0; i < this.edgeCount[n2]; ++i) {
            graphics2D.drawLine(this.nodeX[n2][this.link[n2][i][0]] + n, this.nodeY[n2][this.link[n2][i][0]] + n, this.nodeX[n2][this.link[n2][i][1]] + n, this.nodeY[n2][this.link[n2][i][1]] + n);
        }
        final int n3 = 8;
        if (this.showSteiner.getState()) {
            ...
            for (int j = this.edgeCount[n2]; j < this.curLink[n2]; ++j) {
                graphics2D.drawLine(this.nodeX[n2][this.link[n2][j][0]] + n, this.nodeY[n2][this.link[n2][j][0]] + n, this.nodeX[n2][this.link[n2][j][1]] + n, this.nodeY[n2][this.link[n2][j][1]] + n);
            }
        ...
        for (int n4 = 0; n4 < this.edgeCount[curGraph]; ++n4) {
            graphics2D.drawLine(this.nodeX[curGraph][this.link[curGraph][n4][0]] + n, this.nodeY[curGraph][this.link[curGraph][n4][0]] + n, this.nodeX[curGraph][this.link[curGraph][n4][1]] + n, this.nodeY[curGraph][this.link[curGraph][n4][1]] + n);
        }
        if (this.showSteiner.getState()) {
            graphics2D.setStroke(new BasicStroke(1.0f));
            graphics2D.setColor(Color.blue);
            for (int n5 = this.edgeCount[curGraph]; n5 < this.curLink[curGraph]; ++n5) {
                graphics2D.drawLine(this.nodeX[curGraph][this.link[curGraph][n5][0]] + n, this.nodeY[curGraph][this.link[curGraph][n5][0]] + n, this.nodeX[curGraph][this.link[curGraph][n5][1]] + n, this.nodeY[curGraph][this.link[curGraph][n5][1]] + n);
            }
        ...
        for (int n7 = 0; n7 < this.nodeCount[curGraph]; ++n7) {
            this.drawNode(graphics2D, this.nodeX[curGraph][n7], this.nodeY[curGraph][n7], this.nodeColor[curGraph][n7], this.nodeID[curGraph][n7], 100);
        }
    }
    ...
    public void mouseMoved(final MouseEvent mouseEvent) {
        ...
        if (this.selectNode && n > 15 && n < 580 && n2 > 15 && n2 < 530) {
            ...
            graphics2.drawLine(this.nodeX[this.curGraph][this.arcStart] + 10, this.nodeY[this.curGraph][this.arcStart] + 10, n - 10 + 10, n2 - 10 + 10);
            graphics.drawImage(this.bi, 10, 10, this);
            this.changed = true;
        }
    ...
    public void mousePressed(final MouseEvent mouseEvent) {
        ...
        if ((mouseEvent.getModifiers() & 0x10) == 0x10) {
            ...
            if (arcStart > -1) {
                this.drawNode(graphics, this.nodeX[curGraph3][arcStart] + 10, this.nodeY[curGraph3][arcStart] + 10, this.nodeColor[curGraph3][arcStart], this.nodeID[curGraph3][arcStart], 100);
            }
            this.arcStart = -1;
            graphics.setColor(this.background);
            graphics.drawRect(645, 155, 30, 30);
        ...
        if (this.drawing) {
            ...
            if (b) {
                ...
                if (this.nc > 1) {
                    ...
                    this.curNodes[this.nc++] = i;
                    int j;
                    for (j = 0; j < this.curLink[n5] && (this.link[n5][j][0] != this.curNodes[0] || this.link[n5][j][1] != this.curNodes[this.nc - 1]) && (this.link[n5][j][1] != this.curNodes[0] || this.link[n5][j][0] != this.curNodes[this.nc - 1]); ++j) {}
    ...
    void blink() {
        ...
        this.drawNode(graphics, this.nodeX[curGraph][arcStart], this.nodeY[curGraph][arcStart], this.nodeColor[curGraph][arcStart], this.nodeID[curGraph][arcStart], this.blinkColor);
        graphics.dispose();
    }
    ...
    void addBoundary() {
        ...
        this.rtheta = this.rtheta * 180.0 / 3.141592653589793;
        if (this.rtheta < 0.0) {
            this.rtheta += 360.0;
        }
        ...
        for (int n14 = 0; n14 < 4; ++n14) {
            for (l = 0; l < n; ++l) {
                if (!this.edgeCross(array[n14], array2[n14], this.nodeX[0][l], this.nodeY[0][l], 0, l)) {
                    array5[n14][l] = Math.sqrt((this.nodeX[0][l] - array[n14]) * (this.nodeX[0][l] - array[n14]) + (this.nodeY[0][l] - array2[n14]) * (this.nodeY[0][l] - array2[n14]));
                    for (m = 0; m < 4; ++m) {
                        for (int n15 = 0; n15 < n; ++n15) {
                            if (!this.edgeCross(array3[m], array4[m], this.nodeX[1][n15], this.nodeY[1][n15], 1, n15)) {
                                array6[m][n15] = Math.sqrt((this.nodeX[1][n15] - array3[m]) * (this.nodeX[1][n15] - array3[m]) + (this.nodeY[1][n15] - array4[m]) * (this.nodeY[1][n15] - array4[m]));
                            }
                            else {
    ...
    void preFindBorder(final int[] array, final int[] array2, final int n, final int[][] array3, final int n2, final int n3) {
        ...
        if (n3 < n) {
            ...
            for (int k = 0; k < array5[n3]; ++k) {
                double atan2;
                for (atan2 = Math.atan2(array[array4[n3][k]] - array[n3], array2[array4[n3][k]] - array2[n3]); atan2 < 0.0; atan2 += 6.283185307179586) {}
                if (atan2 > n4) {
                    n4 = atan2;
                    n5 = k;
                }
            ...
            if (array5[n3] > 0) {
                ...
                while (n6 != n3) {
                    ...
                    for (int n10 = 0; n10 < array5[n6]; ++n10) {
                        double n11;
                        for (n11 = Math.atan2(array[array4[n6][n10]] - array[n6], array2[array4[n6][n10]] - array2[n6]) - atan3; n11 < 0.0; n11 += 6.283185307179586) {}
                        if (n11 > n8) {
                            n8 = n11;
                            n9 = n10;
                        }
                ...
                for (int n14 = 0; n14 < bc; ++n14) {
                    graphics2.drawLine(array[array6[n14]] + 10, array2[array6[n14]] + 10, array[array6[(n14 + 1) % bc]] + 10, array2[array6[(n14 + 1) % bc]] + 10);
                    graphics.drawImage(this.bi, 10, 10, this);
                }
                graphics2.dispose();
                graphics.dispose();
            ...
            while (array5[n5] > 0) {
                ...
                do {
                    ...
                    for (int l = 0; l < array5[n8]; ++l) {
                        double n12;
                        for (n12 = Math.atan2(array2[n8] - array2[array4[n8][l]], array[n8] - array[array4[n8][l]]) - atan2; n12 < 0.0; n12 += 6.283185307179586) {}
                        if (n12 < n10) {
                            if (nc > 1) {
                ...
                for (int n19 = 0; n19 < nc; ++n19) {
                    for (int n20 = n19 + 1; n20 < nc; ++n20) {
                        if (array6[n20] == array6[n19]) {
                            ...
                            int n25;
                            for (n25 = n19 + 1; n25 < nc && (array6[n25] != array6[n19] || array6[(n25 + 1) % nc] != array6[(n19 + 1) % nc]) && (array6[n25] != array6[(n19 + 1) % nc] || array6[(n25 + 1) % nc] != array6[n19]); ++n25) {}
                            if (n25 == nc) {
    ...
    void removeRepeated() {
        for (int i = 0; i < this.curLink[0]; ++i) {
            for (int j = i + 1; j < this.curLink[0]; ++j) {
                if ((this.link[0][i][0] == this.link[0][j][0] || this.link[0][i][0] == this.link[0][j][1]) && (this.link[0][i][1] == this.link[0][j][0] || this.link[0][i][1] == this.link[0][j][1])) {
                    final int[] curLink = this.curLink;
                    final int n = 0;
                    --curLink[n];
        ...
        for (int l = 0; l < this.curLink[1]; ++l) {
            for (int n2 = l + 1; n2 < this.curLink[1]; ++n2) {
                if ((this.link[1][l][0] == this.link[1][n2][0] || this.link[1][l][0] == this.link[1][n2][1]) && (this.link[1][l][1] == this.link[1][n2][0] || this.link[1][l][1] == this.link[1][n2][1])) {
                    final int[] curLink2 = this.curLink;
                    final int n3 = 1;
                    --curLink2[n3];
    ...
    void tokenize(final String s) {
        ...
        if (s2.compareTo("node") == 0) {
            final int n3 = n2 + 1;
            final int index = s.indexOf(32, n3);
            final int int1 = Integer.parseInt(s.substring(n3, index));
            final int n4 = index + 1;
            final int index2 = s.indexOf(32, n4);
            final int int2 = Integer.parseInt(s.substring(n4, index2));
            final int n5 = index2 + 1;
            final int index3 = s.indexOf(32, n5);
            final int int3 = Integer.parseInt(s.substring(n5, index3));
            final int n6 = index3 + 1;
            final int index4 = s.indexOf(32, n6);
            final int int4 = Integer.parseInt(s.substring(n6, index4));
            final int n7 = index4 + 1;
            final int index5 = s.indexOf(32, n7);
            final int int5 = Integer.parseInt(s.substring(n7, index5));
            final int n8 = index5 + 1;
            final int index6 = s.indexOf(32, n8);
            final int int6 = Integer.parseInt(s.substring(n8, index6));
            final int n9 = index6 + 1;
            n2 = s.indexOf(32, n9);
            s2 = s.substring(n9, n2);
            final int int7 = Integer.parseInt(s2);
            this.nodeX[int7][int1] = int2;
            this.nodeY[int7][int1] = int3;
            this.nodeColor[int7][int1] = new Color(int4, int5, int6);
            this.nodeID[int7][int1] = int1;
            this.curNode[int7] = int1 + 1;
            if (int4 != 255 || int5 != 255 || int6 != 255) {
                this.nodeCount[int7] = int1 + 1;
            }
        }
        if (s2.compareTo("edge") == 0) {
            final int n10 = n2 + 1;
        ...
        try {
            ...
            for (int i = 0; i < 2; ++i) {
                for (int j = 0; j < this.curNode[i]; ++j) {
                    fileWriter.write("node " + j + " " + this.nodeX[i][j] + " " + this.nodeY[i][j] + " " + this.nodeColor[i][j].getRed() + " " + this.nodeColor[i][j].getGreen() + " " + this.nodeColor[i][j].getBlue() + " " + i + " \r\n");
                }
            }
            for (int k = 0; k < 2; ++k) {
            ...
            while (array5[n5] > 0) {
                ...
                while (l != n5) {
                    ...
                    for (int n10 = 0; n10 < array5[l]; ++n10) {
                        double n11;
                        for (n11 = Math.atan2(array2[array4[l][n10]] - array2[l], array[array4[l][n10]] - array[l]) - atan2; n11 <= 0.0; n11 += 6.283185307179586) {}
                        if (n11 > n8) {
                            if (nc > 1) {
                ...
                for (int n17 = 0; n17 < nc; ++n17) {
                    ...
                    graphics2.drawLine(array[array6[n17]] + 10, array2[array6[n17]] + 10, array[array6[(n17 + 1) % nc]] + 10, array2[array6[(n17 + 1) % nc]] + 10);
                    graphics.drawImage(this.bi, 10, 10, this);
                    try {
                        Thread.sleep(500L);
                    }
    ...
    void redrawDimGraph(final Graphics graphics) {
        ...
        for (int i = 0; i < this.edgeCount[0]; ++i) {
            graphics2D.drawLine(this.nodeX[0][this.link[0][i][0]] + n, this.nodeY[0][this.link[0][i][0]] + n, this.nodeX[0][this.link[0][i][1]] + n, this.nodeY[0][this.link[0][i][1]] + n);
        }
        if (this.showSteiner.getState()) {
            graphics2D.setStroke(new BasicStroke(1.0f));
            graphics2D.setColor(new Color(255 - 255 * value / 100, 255 - 255 * value / 100, 255));
            for (int j = this.edgeCount[0]; j < this.curLink[0]; ++j) {
                graphics2D.drawLine(this.nodeX[0][this.link[0][j][0]] + n, this.nodeY[0][this.link[0][j][0]] + n, this.nodeX[0][this.link[0][j][1]] + n, this.nodeY[0][this.link[0][j][1]] + n);
            }
            graphics2D.setColor(new Color(255 - 255 * value / 100, 200 + 55 * (100 - value) / 100, 255 - 255 * value / 100));
        ...
        for (int n3 = 0; n3 < this.edgeCount[1]; ++n3) {
            graphics2D.drawLine(this.nodeX[1][this.link[1][n3][0]] + n, this.nodeY[1][this.link[1][n3][0]] + n, this.nodeX[1][this.link[1][n3][1]] + n, this.nodeY[1][this.link[1][n3][1]] + n);
        }
        if (this.showSteiner.getState()) {
            graphics2D.setStroke(new BasicStroke(1.0f));
            graphics2D.setColor(new Color(255 - 255 * value / 100, 255 - 255 * value / 100, 255));
            for (int n4 = this.edgeCount[1]; n4 < this.curLink[1]; ++n4) {
                graphics2D.drawLine(this.nodeX[1][this.link[1][n4][0]] + n, this.nodeY[1][this.link[1][n4][0]] + n, this.nodeX[1][this.link[1][n4][1]] + n, this.nodeY[1][this.link[1][n4][1]] + n);
            }
            graphics2D.setColor(new Color(255 - 255 * value / 100, 200 + 55 * (100 - value) / 100, 255 - 255 * value / 100));
    ...
    void linearMotion(final Graphics graphics) {
        ...
        for (int i = 0; i < this.curNode[0]; ++i) {
            ...
            array3[i] = new Color(red + (color2.getRed() - red) * this.curFrame / value, green + (color2.getGreen() - green) * this.curFrame / value, blue + (color2.getBlue() - blue) * this.curFrame / value);
    ...
    void rigid1(final Graphics graphics, final int n, final int n2, final boolean b) {
        ...
        for (int i = 0; i < this.curNode[0]; ++i) {
            ...
            final int blue = color.getBlue();
            final Color color2 = this.nodeColor[1][i];
            array3[i] = new Color(red + (color2.getRed() - red) * n / n2, green + (color2.getGreen() - green) * n / n2, blue + (color2.getBlue() - blue) * n / n2);
    ...
    void calcFramesConvex() {
        ...
        for (int i = 0; i < n; ++i) {
            ...
            if (n3 == n2) {
                ...
                for (int j = 0; j < n; ++j) {
                    if (this.adjMat(i, j)) {
                        array3[j] = Math.atan2(this.nodeX[0][j] - this.nodeX[0][i], this.nodeY[0][j] - this.nodeY[0][i]) + 3.141592653589793;
                        ++n8;
                    }
                    else {
                        array3[j] = 1000.0;
                ...
                for (int n9 = n8, k = 0; k < n9; ++k) {
                    ...
                    array2[n10] = (Math.tan(this.getAngle(n13, n14, this.nodeX[0][n11] - this.nodeX[0][i], this.nodeY[0][n11] - this.nodeY[0][i]) / 2.0) + Math.tan(this.getAngle(n13, n14, this.nodeX[0][n12] - this.nodeX[0][i], this.nodeY[0][n12] - this.nodeY[0][i]) / 2.0)) / Math.sqrt(n13 * n13 + n14 * n14);
                    n7 += array2[n10];
                }
                int n15 = 0;
                for (int l = 0; l < n; ++l) {
                    if (this.adjMat(i, l)) {
                        array3[l] = Math.atan2(this.nodeX[1][l] - this.nodeX[1][i], this.nodeY[1][l] - this.nodeY[1][i]) + 3.141592653589793;
                        ++n15;
                    }
                    else {
                        array3[l] = 1000.0;
                ...
                for (int n16 = n15, n17 = 0; n17 < n16; ++n17) {
                    ...
                    array[n18] = (Math.tan(this.getAngle(n21, n22, this.nodeX[1][n19] - this.nodeX[1][i], this.nodeY[1][n19] - this.nodeY[1][i]) / 2.0) + Math.tan(this.getAngle(n21, n22, this.nodeX[1][n20] - this.nodeX[1][i], this.nodeY[1][n20] - this.nodeY[1][i]) / 2.0)) / Math.sqrt(n21 * n21 + n22 * n22);
                    n6 += array[n18];
                }
            ...
            if (n6 != n2) {
                ...
                if (n4 == 0) {
                    if (this.theta[0][j] < 0.0) {
                        this.theta[0][j] += 6.283185307179586;
                    }
                }
                else {
                ...
                if (n4 == 0) {
                    if (this.theta[1][j] < 0.0) {
                        this.theta[1][j] += 6.283185307179586;
                    }
                }
                else {
    ...
    void ConvexMotion(final Graphics graphics, final int n, final int n2) {
        ...
        for (int n11 = 0; n11 < n3 - n4; ++n11) {
            ...
            for (int n14 = 0; n14 < n4; ++n14) {
                n13 += ((1.0 - n5) * this.lamda0[array8[n11]][array9[n14]] + n5 * this.lamda1[array8[n11]][array9[n14]]) * array4[array9[n14]];
            }
        ...
        for (int n18 = 0; n18 < n3; ++n18) {
            ...
            array[n18] = new Color(red + (color2.getRed() - red) * n / n2, green + (color2.getGreen() - green) * n / n2, blue + (color2.getBlue() - blue) * n / n2);
        }
    ...
    void drawAnimatedGraph(final Graphics graphics, final int[] array, final int[] array2, final Color[] array3) {
        ...
        graphics2D.setStroke(new BasicStroke(2.0f));
        for (int i = 0; i < this.edgeCount[0]; ++i) {
            graphics2D.drawLine(array[this.link[0][i][0]] + n, array2[this.link[0][i][0]] + n, array[this.link[0][i][1]] + n, array2[this.link[0][i][1]] + n);
        }
        if (this.showSteiner.getState()) {
            graphics2D.setStroke(new BasicStroke(1.0f));
            graphics2D.setColor(Color.blue);
            for (int j = this.edgeCount[0]; j < this.curLink[0]; ++j) {
                graphics2D.drawLine(array[this.link[0][j][0]] + n, array2[this.link[0][j][0]] + n, array[this.link[0][j][1]] + n, array2[this.link[0][j][1]] + n);
            }
        }
        graphics2D.setStroke(new BasicStroke(1.0f));
    ...
    int findPoly(final int[] array, final int[] array2, final int n, final int[][] array3, final int n2, final int n3, final int[][] array4, int n4, final int[][] array5, int n5) {
        ...
        for (int k = n3; k < n; ++k) {
            for (int l = 0; l < array7[k]; ++l) {
                double atan2;
                int n6;
                for (atan2 = Math.atan2(array2[array6[k][l]] - array2[k], array[array6[k][l]] - array[k]), n6 = l - 1; n6 >= 0 && atan2 <= array9[n6]; --n6) {}
                ++n6;
    ...
    int findPoly1(final int[] array, final int[] array2, final int n, final int[][] array3, final int n2, final int n3, final int[][] array4, int n4, final int[][] array5, int n5) {
            ...
            while (array7[k] > 0) {
                ...
                while (l != k) {
                    ...
                    for (int n11 = 0; n11 < array7[l]; ++n11) {
                        double n12;
                        for (n12 = Math.atan2(array[array6[l][n11]] - array[l], array2[array6[l][n11]] - array2[l]) - atan2; n12 < 0.0; n12 += 6.283185307179586) {}
                        if (n12 > n9) {
                            n9 = n12;
                            n10 = n11;
                        }
    ...
    int inside(final int n, final int n2, final int n3, final int[] array, final int n4, final int[] array2, final int[] array3) {
        ...
        int n6 = -1;
        for (int i = 0; i < n4; ++i) {
            if ((float)this.getAngle1(n, n2, n3, array[i], array2, array3) == 6.2831855f && n5 > array2[i]) {
                n5 = array2[i];
                n6 = i;
            }
        }
    ...
    void findSteiner1(final int n, final int[] array, final int[] array2, int n2, final int[] array3, final int[] array4, int n3, final int[][] array5, int n4, final int[][] array6, int n5, final int[] array7) {
        ...
        int n7 = n;
        for (int i = 0; i < n; ++i) {
            array10[i] = (int)(200.0 * Math.cos(i * 3.141592653589793 * 2.0 / n) + 250.0);
            array9[i] = (int)(200.0 * Math.sin(i * 3.141592653589793 * 2.0 / n) + 250.0);
            array8[i][0] = i;
            array8[i][1] = (i + 1) % n;
        }
